...
if __name__ == '__main__':
    ...
    parser.add_argument('--dump_path', default="/home/ezdx/文档/code/crag/temp/zhwiki-latest-pages-articles.xml.bz2", type=str)
    parser.add_argument('--seg_size', default=None, type=int)
    parser.add_argument('--stride', default=None, type=int)
    parser.add_argument('--num_workers', default=64, type=int)
    parser.add_argument('--save_path', type=str, default='/home/ezdx/文档/code/crag/scripts/clean_corpus.jsonl')
    ...
    temp_dir = os.path.join(Path(args.save_path).parent, 'temp_clean_corpus')
    # # 创建临时目录
    os.makedirs(temp_dir)

    # 使用wikiextractor从维基百科转储中提取文本，输出为JSON格式，过滤消歧义页面
    # subprocess 是 Python 3.5+ 中用于运行外部命令的推荐方式,它接受一个命令及其参数的列表，并执行该命令
    ...
    for doc in tqdm(nlp.pipe(all_text, n_process=args.num_workers, batch_size=64), total=len(all_text)):
        # 获取当前文档的标题
        title = all_title[idx]
        # 索引递增，指向下一个标题
        idx += 1
