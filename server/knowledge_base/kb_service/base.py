...
class KBService(ABC):
    ...
    async def add_doc(self, kb_file: KnowledgeFile, docs: List[Document] = [], **kwargs):
        ...
        if docs:
            custom_docs = True
            for doc in docs:
                #setdefault是字典对象的方法，与get作用类似，如果如果键不存在于字典中，将会添加键并将值设为默认值
        ...
        if docs:
            ...
            # for doc in docs:
            #     try:
            #         # get第二个方法的意思是如果指定键的值不存在时，返回该默认值
            ...
            # do_add_doc，这个方法在子类重写了，所以运行到这里的时候调用的是子类中的方法放入向量数据库当中
    ...
    async def search_docs(self,
        ...
        # docs = await self.do_search(query, 10, score_threshold)
        return docs
...
class KBServiceFactory:
    ...
    async def get_service_by_name(kb_name: str) -> KBService:
        ...
        """
        # 下划线在此处是kb_name,到数据库中查询这个数据库的相关信息
